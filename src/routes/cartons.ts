...
function parseCartonBody(body: Record<string, string>) {
  const { name, sku, barcode, length_cm, width_cm, height_cm, unit_cost, notes } = body;
  return {
    name: name?.trim(),
    sku: sku?.trim() || null,
  ...
  try {
    await db.execute({
      sql: `INSERT INTO carton_types (id, name, sku, barcode, length_cm, width_cm, height_cm, unit_cost, notes, created_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      args: [ulid(), fields.name, fields.sku, fields.barcode,
    ...
    return res.render("pages/cartons/form", {
      title: "New Carton Type",
      carton: req.body,
      error: constraintMessage(err) ?? "Could not save carton type. Please try again.",
      componentScripts: FORM_SCRIPTS,
    });
  }
...
router.post("/:id/edit", requireRole("admin", "manager"), async (req, res) => {
  const fields = parseCartonBody(req.body);
  if (!fields.name) {
    const result = await db.execute({ sql: "SELECT * FROM carton_types WHERE id = ?", args: [req.params.id] });
    return res.render("pages/cartons/form", {
      title: "Edit Carton Type",
  ...
  try {
    await db.execute({
      ...
      args: [fields.name, fields.sku, fields.barcode,
             fields.length_cm, fields.width_cm, fields.height_cm,
             fields.unit_cost, fields.notes, req.params.id],
    });
  } catch (err) {
    const result = await db.execute({ sql: "SELECT * FROM carton_types WHERE id = ?", args: [req.params.id] });
    return res.render("pages/cartons/form", {
