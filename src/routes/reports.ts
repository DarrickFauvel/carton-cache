...
router.get("/", requireAuth, async (req, res) => {
  ...
  // Stock value (new cartons on hand × unit cost)
  const stockValue = await db.execute(`
    SELECT ct.name, il.condition, il.quantity, il.location_id,
           ct.unit_cost, il.quantity * ct.unit_cost AS value
    FROM inventory_lots il
    JOIN carton_types ct ON ct.id = il.carton_type_id
    WHERE ct.unit_cost IS NOT NULL
    ORDER BY ct.name
  `);

  // Reuse savings: received non-new cartons × unit_cost
  const reuseSavings = await db.execute({
    sql: `
      SELECT ct.name, t.condition, SUM(t.quantity) AS qty,
  ...
  // Spend on new cartons
  const spend = await db.execute({
    sql: `
      SELECT ct.name, SUM(t.quantity) AS qty,
  ...
  // Weekly consumption by carton type (last 12 weeks)
...
router.get("/snapshot.csv", requireAuth, async (_req, res) => {
  const result = await db.execute(`
    SELECT l.name AS location, ct.name AS carton, ct.sku, il.condition, il.quantity
    FROM inventory_lots il
    JOIN carton_types ct ON ct.id = il.carton_type_id
